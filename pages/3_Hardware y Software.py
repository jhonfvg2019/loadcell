...
st.title('Diagrama esquematico')
with open("static/schematic.svg") as f:
    svg1 = f.read()
st.image(svg1)


st.title('Código fuente')

st.write(
    """\nEste código corresponde a un sistema para medir y registrar cargas aplicadas en 7 celdas de carga de forma concurrente:\n\n - Se incluyen bibliotecas para comunicarse con las celdas HX711, con una LCD, RTC y tarjeta SD.\n\n - Se definen las conexiones de cada celda a diferentes pines y objetos para acceder a ellas.\n\n - Se crean variables para almacenar los datos, estado y nombre de archivo.\n\n - En setup se inicializan los periféricos, calibran las celdas, y crean 8 tareas para medir cada celda de forma independiente.\n\n - La función loop actualiza la pantalla LCD mostrando los datos.\n\n - Las tareas Task1-Task7 leen en paralelo los valores de cada celda de manera asíncrona.\n\n - La tarea Task8 captura los datos cuando se presiona el botón, los guarda en SD y reinicia.\n\n- El menú permite calibrar celdas individualmente y configurar parámetros.\n\nEl código implementa de forma concurrente y asíncrona la medición, visualización y almacenamiento de datos de las 7 celdas de carga utilizando tareas para optimizar el proceso.\n"""
)
...
float loadCell1, loadCell2, loadCell3, loadCell4, loadCell5, loadCell6, loadCell7;
bool endCell1 = false;
bool endCell2 = false;
bool endCell3 = false;
bool endCell4 = false;
...
void loop() {
  ...
  if (start)
    state = "Start";
  LCDI.lcdLine(time, "c1: " + String(loadCell1, 1), "c2: " + String(loadCell2, 1), "c3: " + String(loadCell3, 1), "c4: " + String(loadCell4, 1), "c5: " + String(loadCell5, 1), "c6: " + String(loadCell6, 1), "c7: " + String(loadCell7, 1), state);
  delay(1000);
}
...
void Task8code(void* parameter) {
  ...
  for (;;) {
    ...
    if (endCell1 && endCell2 && endCell3 && endCell4 && endCell5 && endCell6 && endCell7 && start) {
      ...
      loadCellsValues = String(elapsedTime) + ","  + String(loadCell1, 1) + "," + String(loadCell2, 1) + "," + String(loadCell3, 1) + "," + String(loadCell4, 1) + "," + String(loadCell5, 1) + "," + String(loadCell6, 1) + "," + String(loadCell7, 1) + "\n";
